...
var dadataSuggestions = {
    ...
    init: function () {
        ...
        form_names.forEach(function (value, index, a) {
            if ($("#" + value).length) {
                ...
                $("#" + value).append('<input type="hidden" value="1" name="' + subName + '"></input>');
            }
        });
        if (dadataSuggestions.configuration.DADATA_SUGGESTIONS_FIO)
            dadataSuggestions.configuration.fio_fields.forEach(function (value, index, a) {
                ...
                if (nameEl.length && !$("#" + value.suggest_field).length) {
                    nameEl.parent().before(dadataSuggestions.generateInputHTML(value.suggest_field, dadataSuggestions.configuration.suggest_fio_label, dadataSuggestions.configuration.suggest_fio));
    ...
    generateInputHTML: function (id, label, value) {
        var output = '<div class=\"required form-group\">';
        output += '<label for=\"' + id + '\">' + label + '<sup>*</sup></label>';
        output += '<input type=\"text\" id=\"' + id + '\" class=\"required form-control\" value=\"' + value + '\">';
